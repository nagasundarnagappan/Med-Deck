...
def saveToIpfs(value):
    res = api.add_pyobj(value)
    df = pd.DataFrame({'hash': res}, index=[0])
    df.to_csv('datas.csv', mode='a', index=False, header=False)
    return res


def getFromIpfs(hash):
    return api.get_json(hash)
...
def main():
    ...
    patient_type = st.radio(
        "Patient Type ?", ('Returned Home', 'Hospitalized'))
    ...
    if intubed == 'Yes':
        ...
        INTUBED = 2
    pneumonia = st.radio(
        "Does the patient already have air sacs inflammation ?", ('Yes', 'No'))
    if pneumonia == 'Yes':
        PNEUMONIA = 1
    else:
        PNEUMONIA = 2
    ...
    if diabetes == 'Yes':
        ...
        DIABETES = 2
    copd = st.radio(
        "Does the patient has Chronic Obstructive Pulmonary disease ?", ('Yes', 'No'))
    if copd == 'Yes':
        COPD = 1
    else:
        COPD = 2
    ...
    if hyp == 'Yes':
        ...
        HYP = 2
    cardio = st.radio(
        "Does the patient has Heart or Blood Vessels related disease ?", ('Yes', 'No'))
    if cardio == 'Yes':
        CARDIOVASCULAR = 1
    else:
        CARDIOVASCULAR = 2
    ...
    if obese == 'Yes':
        ...
        OBESITY = 2
    renal_chronic = st.radio(
        "Does the patient has Chronic Renal disease ?", ('Yes', 'No'))
    ...
    if st.button("Submit"):
        result = prediction(SEX, PATIENT_TYPE, INTUBED, PNEUMONIA, AGE, PREGNANT, DIABETES,
                            COPD, ASTHMA, INMSUPR, HYP, OD, CARDIOVASCULAR, OBESITY, RENAL_CHRONIC, TOBOCCO)
